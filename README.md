...
- **NVD platform CVEs not in CSAF** — [Catalog of NVD `tianocore:edk2` CVEs](docs/nvd-edk2-platform-cves-not-in-csaf.md) that have no published GHSA and no reliable `edk2-stableYYYYMM` mapping; they are **not** listed in quarterly CSAF until versioning can be determined (reconcile the doc when advisories or NVD change).
